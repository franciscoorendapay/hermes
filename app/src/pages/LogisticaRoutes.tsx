...
export default function LogisticaRoutes() {
  ...
  const fetchOrdens = async () => {
    try {
      ...
      setOrdens(data.filter(o => o.status === "pendente" || o.status === "em_andamento"));
    } catch (err) {
  ...
  const handleSaveRoute = async () => {
    try {
      await Promise.all(
        selectedOrdensData.map(o =>
          logisticaService.atualizarStatus(o.id, o.tipo, "em_andamento")
        )
      );
      setRouteSaved(true);
  ...
  const formatEndereco = (leads: OrdemLogistica["leads"]) => {
    ...
    return parts.length > 0 ? parts.join(", ") : leads.endereco_cidade || "Endereço não disponível";
  };
